...
impl fmt::Display for DecisionReason {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::DefaultChinese => write!(f, "default_chinese"),
        }
    }
}
...
pub fn classify(context: &LineContext) -> Decision {
    ...
    match (previous, next) {
        ...
        _ => Decision {
            mode: InputMode::Chinese,
            reason: DecisionReason::DefaultChinese,
        },
    }
}
...
mod tests {
    ...
    #[test]
    fn falls_back_to_chinese_when_no_signal_exists() {
        let decision = classify(&context("12345", 2)); //  cursor is after the number character '2'
        assert_eq!(decision.mode, InputMode::Chinese);
        assert_eq!(decision.reason, DecisionReason::DefaultChinese);
    }
}

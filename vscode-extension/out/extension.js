...
const MONITOR_VIEW_ID = 'smartShift.monitorView';
const LEGACY_MONITOR_VIEW_TYPE_PREFIX = 'smartShiftMonitor-';
const LEGACY_MONITOR_VIEW_TYPE_PREFIX_LEGACY = 'smartShiftDebugView';
...
/** 只跟踪用户代码编辑器里的文档，忽略 Output/Git/虚拟文档等。 */
function isTrackableTextDocument(document) {
    const scheme = document.uri.scheme;
    if (scheme === 'output' || scheme === 'git' || scheme === 'vscode-chat-code-block') {
        return false;
    }
    const base = path.basename(document.fileName);
    if (base.startsWith('extension-output-') ||
...
function logEvent(type, detail, line, cursor) {
    const now = Date.now();
    lastActivityAt = now;
    eventCounter++;
    const entry = { timestamp: now, type, detail, line, cursor };
...
function updateStatusBar() {
    ...
    statusBarItem.tooltip = '点击打开 Smart Shift 侧边栏';
}
function updateCachedState(editor) {
...
function getPanelState() {
    const editor = vscode.window.activeTextEditor;
    const info = editor ? getLineInfo(editor) : null;
    return {
        status: info
            ? {
                composing: isComposing,
...
let monitorViewProvider;
function refreshMonitorView() {
    monitorViewProvider?.refresh();
}
class SmartShiftMonitorViewProvider {
    _view;
    _disposables = [];
    ...
    refresh() {
        this._view?.webview.postMessage(getPanelState());
    }
    reveal() {
        vscode.commands.executeCommand(`${MONITOR_VIEW_ID}.focus`);
        this.refresh();
    }
    _getHtml() {
        return `<!DOCTYPE html>
...
/** Cursor/VS Code may prefix webview viewType with mainThreadWebview-. */
function normalizeWebviewViewType(viewType) {
    const prefix = 'mainThreadWebview-';
    return viewType.startsWith(prefix) ? viewType.slice(prefix.length) : viewType;
}
function isCurrentMonitorViewType(viewType) {
    if (!viewType) {
        return false;
    }
    const normalized = normalizeWebviewViewType(viewType);
    return normalized === MONITOR_VIEW_ID || viewType.includes(MONITOR_VIEW_ID);
}
function isAnyMonitorViewType(viewType) {
    if (!viewType) {
        return false;
    }
    const normalized = normalizeWebviewViewType(viewType);
    ...
    return (normalized.startsWith(LEGACY_MONITOR_VIEW_TYPE_PREFIX) ||
        normalized.startsWith(LEGACY_MONITOR_VIEW_TYPE_PREFIX_LEGACY) ||
    ...
    try {
        for (const group of vscode.window.tabGroups.all) {
            for (const tab of group.tabs) {
                ...
                const shouldClose = isAnyMonitorViewType(viewType) ||
...
function activate(context) {
    ...
    // Monitor 不参与 Webview 恢复；启动时仅关闭历史遗留 viewType 的 tab（不用 label 误关当前面板）。
    ...
    monitorViewProvider = new SmartShiftMonitorViewProvider();
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(MONITOR_VIEW_ID, monitorViewProvider, {
        webviewOptions: {
            retainContextWhenHidden: true
        }
    }));
    logToChannel('[Activate] monitor sidebar registered');
    ...
    const statusCommand = vscode.commands.registerCommand('smart-shift.status', () => {
        ...
        if (editor) {
            ...
            vscode.window.showInformationMessage(`Smart Shift: 行 ${info.lineNumber + 1}, 光标 ${info.cursor}, composing=${info.composing}`);
        }
        else {
    ...
    if (initialEditor) {
        ...
        logEvent('selection', '初始化: ' + path.basename(initialEditor.document.fileName), initialEditor.selection.active.line, initialEditor.selection.active.character);
    }
    updateStatusBar();
...
function startPipeServer() {
    ...
    pipeServer = net.createServer((socket) => {
        ...
        socket.on('error', (err) => { logToChannel(`[Pipe] socket error: ${err.message}`); });
        socket.on('close', () => { logToChannel('[Pipe] client disconnected'); buffer = ''; });
    });
    ...
    if (process.platform === 'win32') {
        try {
            ...
            setTimeout(() => { pipeServer?.listen(PIPE_PATH); }, 1000);
        }
    }
    else {
        pipeServer.listen(PIPE_PATH);
...
function deactivate() {
    ...
    monitorViewProvider?.dispose();
    monitorViewProvider = undefined;

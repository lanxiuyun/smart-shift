...
/**
 * ViewType prefix for Monitor panels. A timestamp suffix is appended on each
 * activation so Cursor's Electron process never reuses a cached ServiceWorker
 * from a previous session.
 */
const MONITOR_VIEW_TYPE_PREFIX = 'smartShiftMonitor-';
/** Set once in activate(); changes every Cursor session. */
...
interface EventLog {
    timestamp: number;
    type: 'selection' | 'document' | 'command' | 'pipe' | 'deactivate';
    detail: string;
    line?: number;
    cursor?: number;
}
...
let cachedLineInfo: LineInfo | null = null;
let lastEditorState: { line: number; character: number; lineText: string; timestamp: number } | null = null;
...
class SmartShiftMonitorPanel {
    public static currentPanel: SmartShiftMonitorPanel | undefined;
...
<script>
  ...
  function renderLineWithCursor(line, cursor) {
    ...
    /** Called when the webview tab is closed; do not call panel.dispose() again. */
    ...
    /** Explicitly close the monitor panel from extension code. */
    public dispose() {
        if (SmartShiftMonitorPanel.currentPanel === this) {
            SmartShiftMonitorPanel.currentPanel = undefined;
        }
    ...
    try {
        for (const group of vscode.window.tabGroups.all) {
            for (const tab of group.tabs) {
                ...
                // Keep only the panel that is currently alive in this session.
